...
Use these criteria to resolve missing strategy, content, design, motion, stack and acceptance decisions. Existing adequate specifications satisfy the corresponding criteria; this guide does not require a new planning or approval stage.

## Applicable build checks

For a new premium build or substantial redesign, address the gaps relevant to the request. Reuse completed decisions and existing evidence; the root skill controls scope.
...
When the required design decisions are missing, capture them in the existing spec or the smallest useful new record. The following are available fields, not mandatory headings for every task:
...
- `Reference Contract`: concrete reference behaviors to borrow or avoid where useful, and which references are verified vs user shorthand.
...
- Inspect for interchangeable choices that conflict with the brief. Preserve functional conventions and intentional repeated components; do not remove useful UI merely because other sites also use it.
