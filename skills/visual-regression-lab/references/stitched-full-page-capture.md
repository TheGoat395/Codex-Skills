...
Use `--suppress-fixed-after-first` only when a fixed/sticky overlay would otherwise repeat in every slice. That option changes capture presentation; record it and compare against live behavior.
